...
def main() -> int:
    ...
    seen_companies: dict[str, set[str]] = {"train": set(), "test": set()}
    stats: dict[str, dict] = {}
    ...
    for split in ("train", "test"):
        ...
        stats[split] = {
            "kayit": len(rows),
            "sirket": len(seen_companies[split]),
            ...
            "atlanan_isaretli": skipped_flagged,
    ...
    overlap = seen_companies["train"] & seen_companies["test"]
    ...
    print(f"\nSIRKET ORTUSMESI: {len(overlap)}  {'<- SIZINTI!' if overlap else '(temiz)'}")
    if overlap:
